...
export async function GET(request) {
  try {
    ...
    if (!emailCookie && !authCookie) {
      ...
      return NextResponse.json({ tier: 'free', status: 'none', isSubscribed: false });
    }

    // If email cookie exists, verify it matches the queried email
    if (emailCookie) {
      const cookieEmail = decodeURIComponent(emailCookie[1]).toLowerCase().trim();
    ...
    const { data: subscriber } = await supabase
      .from('subscribers')
      .select('tier, status, current_period_end, cancel_at_period_end, subscriber_number, created_at')
      .eq('email', cleanEmail)
      .single();
