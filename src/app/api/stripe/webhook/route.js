...
async function handleCheckoutCompleted(session, stripe) {
  ...
  if (
    meta.eventSlug ||
    meta.orderNumber?.startsWith("MTIX-") ||
    meta.type === "bundle" ||
    meta.bundle_slug
  ) {
  ...
  // Skip COLOR MY BOOK checkout sessions — CMB uses the same Stripe account
  // for its $3 book paywall, but fulfillment happens in CMB's own precheck
  // + generate-book-background pipeline (not Printful/Printify). Without
  // this filter MSL tries to match CMB's book product against Printful and
  // fires ORDER FAILED | NONE MATCHED alerts (real incident 2026-08-08).
  if (meta.app === "colormybook" || meta.type === "book_purchase") {
    console.log("Skipping COLOR MY BOOK session:", session.id);
    return;
  }
  ...
  try {
    ...
    const shipping =
      rawShipping && rawShipping.address && rawShipping.address.line1
        ? rawShipping
        : fullSession.customer_details;
    const hasShipping = shipping && shipping.address && shipping.address.line1;
    if (!hasShipping) {
      console.warn(
        "No shipping address found in session — emails will still be sent",
      );
    }
    ...
    if (merchItems.length === 0) {
      ...
      if (subItems.length > 0) {
        ...
        if (customerEmail) {
          await registerNewSubscriber(
            customerEmail,
            subItems[0]?.price?.unit_amount || 0,
          );
        }
      }
      return;
    ...
    const customerEmail =
      fullSession.customer_details?.email || session.customer_email;
    const customerName =
      shipping?.name || fullSession.customer_details?.name || "Customer";
    ...
    try {
      if (metadata.printful_items)
        parsedPrintfulItems = JSON.parse(metadata.printful_items);
    } catch (e) {
      console.error("Failed to parse printful_items:", e);
      metadataParseError = `Printful: ${e.message}`;
    }
    try {
      if (metadata.printify_items)
        parsedPrintifyItems = JSON.parse(metadata.printify_items);
    } catch (e) {
      console.error("Failed to parse printify_items:", e);
      metadataParseError =
    ...
    if (
      metadataParseError &&
      (metadata.printful_items || metadata.printify_items)
    ) {
      sendOrderFailedAlert({
      ...
      if (supabase) {
        ...
        if (insertError) {
          ...
          if (insertError.code === "23505") {
            console.log(
              "Duplicate webhook (constraint hit) — original handler already running for:",
              session.id,
            );
            return;
          }
          ...
          console.error(
            "Failed to log order to merch_orders (non-blocking):",
            insertError.message,
          );
        } else {
    ...
    async function updateOrderRecord(updates) {
      ...
      try {
        ...
        console.error("Failed to update merch_orders:", e.message);
      }
    }
    ...
    if (parsedPrintfulItems && parsedPrintfulItems.length > 0 && hasShipping) {
      ...
      for (let attempt = 1; attempt <= 2; attempt++) {
        try {
          console.log(
            `Creating Printful order (attempt ${attempt}):`,
            JSON.stringify(printfulOrder, null, 2),
          );
          ...
          await updateOrderRecord({
            ...
            printful_status: order.status || "created",
          });
          ...
          if (attempt === 2) {
            ...
            sendOrderFailedAlert({
              ...
              console.error("Printful fail alert email error:", err),
            );
          } else {
      ...
      sendOrderFailedAlert({
        ...
        console.error("Printful no-shipping alert error:", err),
      );
      await updateOrderRecord({
        printful_status: "failed",
        printful_error: "No shipping address",
      });
    }
    ...
    if (parsedPrintifyItems && parsedPrintifyItems.length > 0 && hasShipping) {
      ...
      try {
        ...
        await updateOrderRecord({
          printify_order_id: createdOrder.id,
          printify_status: "created_pending_production",
        });
      } catch (e) {
        const msg = e.message || "";
        const is409 =
          msg.includes("Printify API error 409") || msg.includes("409");
        if (is409) {
          console.warn(
            `Printify create returned 409 — looking up existing order by external_id ${printifyOrder.external_id}`,
          );
          try {
            const existing = await printify.findOrderByExternalId(
              printifyOrder.external_id,
            );
            if (existing) {
              console.log(
                `Recovered existing Printify order ${existing.id} for external_id ${printifyOrder.external_id}`,
              );
              createdOrder = existing;
              await updateOrderRecord({
                printify_order_id: existing.id,
                printify_status:
                  existing.status === "on-hold"
                    ? "created_pending_production"
                    : "created",
              });
            } else {
              console.error(
                `409 received but order not found by external_id — manual triage required`,
              );
              await updateOrderRecord({
                printify_status: "failed",
                printify_error: `409 with no recoverable order for external_id ${printifyOrder.external_id}: ${msg}`,
              });
              sendOrderFailedAlert({
                provider: "Printify",
                ...
                console.error("Printify 409 alert error:", err),
              );
            }
          } catch (lookupErr) {
            ...
            await updateOrderRecord({
              printify_status: "failed",
              printify_error: `409 lookup failed: ${lookupErr.message}`,
            });
            sendOrderFailedAlert({
              provider: "Printify",
              ...
              console.error("Printify 409 lookup alert error:", err),
            );
          }
        } else {
          ...
          sendOrderFailedAlert({
            ...
            console.error("Printify create-fail alert error:", err),
          );
        }
      }
      ...
      if (
        createdOrder?.id &&
        createdOrder.status !== "in_production" &&
        createdOrder.status !== "in-production"
      ) {
        try {
          console.log(
            `Sending Printify order ${createdOrder.id} to production`,
          );
          await printify.sendToProduction(createdOrder.id);
          printifyResult = {
            printifyOrderId: createdOrder.id,
          ...
          console.error(
            `Printify send_to_production failed for ${createdOrder.id}: ${msg}`,
          );
          // Code 8502 means "already sending" — actually a success in disguise.
          if (
            msg.includes("8502") ||
            msg.toLowerCase().includes("sending_to_production_delegate")
          ) {
            console.log(
              "send_to_production already in progress at Printify — treating as success",
            );
            printifyResult = {
              printifyOrderId: createdOrder.id,
              sent_to_production: true,
            ...
            await updateOrderRecord({
              ...
              printify_error: `send_to_production failed: ${msg}. Order ${createdOrder.id} is at Printify on-hold; push manually from dashboard.`,
            });
            sendOrderFailedAlert({
              provider: "Printify",
              ...
              console.error("Printify on-hold alert error:", err),
            );
            printifyResult = {
              printifyOrderId: createdOrder.id,
      ...
      sendOrderFailedAlert({
        ...
        console.error("Printify no-shipping alert error:", err),
      );
      await updateOrderRecord({
        printify_status: "failed",
        printify_error: "No shipping address",
      });
    }

    // Update overall order status
    const overallStatus =
      (!parsedPrintfulItems || printfulResult) &&
      (!parsedPrintifyItems || printifyResult)
        ? "fulfilled"
    ...
    if (customerEmail && totalAmount >= 10000) {
      sendBigSpenderThankYou({
        ...
        console.error("Big spender thank you email failed:", err),
      );
    }
      ...
      try {
        ...
        if (supabase) {
          ...
          if (existing) {
            ...
            const currentExpiry = existing.purchased_sub_until
              ? new Date(existing.purchased_sub_until)
              : new Date(0);
    ...
    if (printfulResult || printifyResult) {
      ...
      console.error(
        "Could not match any items with Printful or Printify products",
      );
      ...
      sendOrderFailedAlert({
        ...
        error:
          "Customer paid but NO items matched Printful or Printify. Manual fulfillment required. Check metadata and line items.",
        customerName,
        customerEmail,
        items: itemsForEmail,
        total: totalAmount,
...
async function handleSubscriptionCheckout(session, stripe) {
  ...
  try {
    ...
    const { error: upsertErr } = await supabase
      .from("subscribers")
      .upsert(fullRow, { onConflict: "email" });
    if (upsertErr) {
      // Columns may not exist yet — fall back to base columns only
      console.warn(
        "Full upsert failed (migration pending?), falling back:",
        upsertErr.message,
      );
...
async function handleInvoicePaid(invoice) {
  ...
  try {
    ...
    const { tierFromPriceId, normalizeTier, tierLevel } =
    ...
    if (existing) {
      ...
      const newTier =
        incomingLevel >= existingLevel ? tier : normalizeTier(existing.tier);
      await supabase
        .from("subscribers")
        .update({
          status: "active",
...
async function handleSubscriptionCanceled(subscription) {
  ...
  try {
    ...
    email =
      !lookupErr && subscriber?.email
        ? subscriber.email
        : subscription.metadata?.email;
    if (!email) {
      console.log(
        "subscription.deleted: Cannot find subscriber for:",
        subscription.id,
      );
      return;
    }
    ...
    if (creatorSub) {
      await supabase
        ...
        .eq("id", creatorSub.id);
      console.log(
        `[webhook] Creator subscription canceled: ${creatorSub.slug}`,
      );
    }
  } catch (err) {
...
async function registerNewSubscriber(customerEmail, amountCents) {
  ...
  try {
    ...
    console.log(
      `New subscriber registered: ${email} (#${subscriberNumber}, ${tier})`,
    );
...
async function handleSubscriptionUpdated(subscription) {
  ...
  try {
    ...
    if (!subByIdErr && subById) {
      ...
      if (!metaEmail) {
        console.log(
          "subscription.updated: No subscriber found for sub:",
          subscription.id,
        );
        return;
      }
    ...
    const newTier = normalizeTier(
      priceId ? tierFromPriceId(priceId) : subscriber.tier,
    );
    const isActive =
      subscription.status === "active" || subscription.status === "trialing";
    ...
    if (newTier !== subscriber.tier) {
      console.log(
        `Tier changed: ${subscriber.email} ${subscriber.tier} → ${newTier}`,
      );
    }
