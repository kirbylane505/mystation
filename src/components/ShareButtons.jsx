...
export default function ShareButtons({
  ...
  const shareToInstagram = async () => {
    if (navigator.share) {
      try {
        await navigator.share({ title: track?.title || 'MyStation', text: shareText, url: shareUrl });
        return;
      } catch {}
    }
    // Fallback: copy link for pasting in Instagram
  ...
  const shareToSnapchat = async () => {
    ...
    // Fallback: copy link for pasting in Snapchat
    try { await navigator.clipboard.writeText(shareUrl); } catch {}
    setCopied(true);
    setTimeout(() => setCopied(false), 3000);
  };
        ...
        <>
          ...
          <div className="absolute bottom-full mb-2 right-0 w-48 glass rounded-xl overflow-hidden z-50 animate-fade-in">
            <div className="p-2">
              ...
              <button
                onClick={() => { shareToSnapchat(); setShowDropdown(false); }}
                className="w-full flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-white/10 transition"
              >
